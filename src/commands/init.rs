...
fn discover_config(env: &HashMap<String, String>, remote_url: Option<&str>) -> Discovered {
    ...
    if let Some(remote) = remote_url {
        if let Some((url, project)) = parse_ado_remote(remote) {
            ...
            let project_id = get(env, &["GITLAB_PROJECT_ID"]).and_then(|v| v.parse::<u64>().ok());
            d.provider = Some(DiscoveredProvider::GitLab {
                token,
                namespace,
                project_id,
            });
    ...
    if d.provider.is_none() {
        if get(env, &["ADO_PAT", "ADO_TOKEN", "ADO_URL"]).is_some() {
            let url = get(env, &["ADO_URL"])
                .unwrap_or_else(|| "https://dev.azure.com/your-org".to_string());
...
fn build_toml(d: &Discovered, remote_url: Option<&str>) -> String {
    let mut out =
        String::from("# Flow Manager configuration — generated by `fm init --discover`\n");
    ...
    match &d.provider {
        ...
        Some(DiscoveredProvider::GitLab {
            token,
            namespace,
            project_id,
        }) => {
...
pub async fn run(path: Option<String>, discover: bool) -> Result<()> {
    ...
    {
        println!(
            "Some fields could not be discovered — edit {} to fill in the placeholders.",
            file_path
        );
    }
